...
export class RateService {
  ...
  async getRate(asset: string, currency: string): Promise<number> {
    const rates = await this.getRates(currency);

    if (!rates[asset]?.[currency]) {
      this.logger.warn(`Rate not found for ${asset} in ${currency}`, 'getRate');
      throw new Error(`Rate not found for ${asset} in ${currency}`);
    }

    return rates[asset][currency];
  }
  ...
  async refreshRatesJob() {
    ...
    for (const currency of currencies) {
      try {
        ...
        this.logger.log(
          `Successfully refreshed rates for ${currency}`,
          'CronJob-end',
        );
      } catch (error) {
        this.logger.error(
          `Failed to refresh rates for ${currency}`,
          'CroJob-end',
        );
      }
    }
